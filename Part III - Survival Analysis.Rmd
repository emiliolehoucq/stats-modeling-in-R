...
`survreg` learns the parameter value, $\lambda$, and the regression coefficients. As an example, we will be consider the model: `surv ~ 1 + obstruct + adhere + nodes.ds`.
...
```{r}
subject_two = list(obstruct = factor('obstruct'), adhere = factor('no adhere'), nodes.ds = factor('<3'))
...
```{r}
survregLogNormal = survreg(surv ~ 1 + obstruct + adhere + nodes.ds,
