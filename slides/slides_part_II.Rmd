...
- a link function that relates the mean of the random to the systematic component
$$\log\left(\frac{\pi_i}{1-\pi_i}\right) = \sum_{j=i}^{n} \beta_j x_{ij}.$$ $\log\left(\pi_i/(1-\pi_i)\right)$ is known as the log odds.
...
where 
$$  X_{1i} = \begin{cases} 1 & \text{if } i\text{th data point is versicolor}\\ 0 &  \text{otherwise}\end{cases},$$
$$\, X_{2i} = \begin{cases} 1 & \text{if } i \text{th data point is virginica}\\ 0 & \text{otherwise}\end{cases}$$ and $X_{3i} = \text{Sepal.Length of the }i\text{th  data point}.$
...
### Deviance
 ...
 For general linear models, we use *deviance* to compare model pairs. Deviance is the difference in log likelihood of the models multipled by 2.
...
If the fitted model is behaves similiar to the saturated model, then the deviance can be well approximated by a chi-squared distribution with $m-n$ degrees of freedom. $m$ is the number of the data points and $n$ is the number of coefficients in our fitted model.
...
A possion GLM is most useful when studying data in which the mean and variance are approximately equal. If they are not not equal, the standard error of the model terms must adjusted to account for the assumption violation.
...
## Data
 ...
 The data set also contains demographic data associated with each student. The data set has five variables:
...
where 
$$X_{1i} = \begin{cases}1 & \text{if mar = Married } \\ 0 & \text{otherwise} \end{cases}.$$
...
plot(as.numeric(row.names(single_bioChemists)),single_bioChemists$art,ylab='number of articles',xlab = 'Index', main="Articles distribution for single students")

poisson_model_single_data = poisson_model$fitted[bioChemists$mar=='Single']

points(as.numeric(row.names(data.frame(poisson_model_single_data[single_bioChemists$kid5== 0]))),
       poisson_model_single_data[single_bioChemists$kid5== 0],col="blue",pch=1)

points(as.numeric(row.names(data.frame(poisson_model_single_data[single_bioChemists$kid5== 1]))),
       poisson_model_single_data[single_bioChemists$kid5== 1],col="yellow",pch=1)

points(as.numeric(row.names(data.frame(poisson_model_single_data[single_bioChemists$kid5== 2]))),
       poisson_model_single_data[single_bioChemists$kid5== 2],col="red",pch=1)
points(as.numeric(row.names(data.frame(poisson_model_single_data[single_bioChemists$kid5== 3]))),
       poisson_model_single_data[single_bioChemists$kid5== 3],col="green",pch=1)
```
\normalsize
...
plot(as.numeric(row.names(mar_bioChemists)),
     mar_bioChemists$art,ylab='number of articles',xlab = 'Index', main="Articles distribution for married students")

poisson_model_married_data = poisson_model$fitted[bioChemists$mar=='Married']

points(as.numeric(row.names(data.frame(poisson_model_married_data[mar_bioChemists$kid5== 0]))),
       poisson_model_married_data[mar_bioChemists$kid5== 0],
       col="blue",pch=2)
points(as.numeric(row.names(data.frame(poisson_model_married_data[mar_bioChemists$kid5== 1]))),
       poisson_model_married_data[mar_bioChemists$kid5== 1],
       col="yellow",pch=2)
points(as.numeric(row.names(data.frame(poisson_model_married_data[mar_bioChemists$kid5== 2]))),
       poisson_model_married_data[mar_bioChemists$kid5== 2],
       col="red",pch=2)
points(as.numeric(row.names(data.frame(poisson_model_married_data[mar_bioChemists$kid5== 3]))),
       poisson_model_married_data[mar_bioChemists$kid5== 3],
       col="green",pch=2)
```
...
library(knitr)
art_binary <- sapply(bioChemists$art,
                                 function(x) ifelse(x > 1, 1, 0))
ment_binary <- sapply(bioChemists$ment,
                                  function(x) 
                                    ifelse(x > median(bioChemists$ment),
                                           1, 0))

bioChemists$art_binary <- factor(art_binary, levels=c(0,1),label=c("low articles  published","high articles published"))
bioChemists$ment_binary <- factor(ment_binary, levels=c(0,1),label=c("low mentor articles ","high mentor articles"))
kable(table(art_relative=bioChemists$art_binary,ment=bioChemists$ment_binary))

```

\pause{Log-linear model to model the group mean count of each cell of the contingency table} 

\pause{Log-linear models allow us to model association between two or more variables.}

\pause{There are no well defined explanatory/response variables. This is because we are more focused on the interaction between two variables.}
...
We would like to learn a contigency table for the `biochemist` data set using number of articles published, number of mentor articles published and number of children.
...
- \pause{a systematic component, $\eta_{ij}$, that relates the explanatory variables,
$$\eta_{ij} = \beta_0+ \sum_{k=1}^{n} \sum_{i=1}^{m-1} \beta_{ki} X_{ki}$$
where $n$ is the number of covariates, $m$ is the number of covariate class of $X_{k}$ and $X_{ki}$ is the total number of observations with $i$th covariate class of $X_{k}$}
-  \pause{a link function that relates the mean of the random to the systematic component
$$\log \mu_{ij} =\beta_0 + \sum_{k=1}^{n} \sum_{i=1}^{m-1} \beta_{ki} X_{ki}$$}
...
Often you'll have to merge the different levels of data and center the predictors.
...
school_data$sm_ses_grandmean <- school_data$meanses - 
  mean(school_data$meanses) # Grand-mean centered school SES
```
...
Above we grand-mean centered student SES and school SES, here we're group-mean centering student SES. Which one you use affects the interpretation of the intercept.
...
We'll estimate the same models with `lmer()` (lme4) and `lme()`nlme.
...
Here we add a cross-level initeraction as a fixed effect.
